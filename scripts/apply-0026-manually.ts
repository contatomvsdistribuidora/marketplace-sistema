/**
 * Aplica manualmente a migration 0026 (drizzle/0026_product_cache_dim_cost.sql)
 * no MySQL apontado por DATABASE_URL.
 *
 * Fase 7.B do plano de cache local BaseLinker: adiciona 11 colunas a
 * product_cache (3 dim + 2 cost + 6 timestamps last-write-wins).
 *
 * Mesmo padrão de apply-0023/0024/0025: usa stored procedure idempotente
 * com checagem em information_schema (MySQL 9 não suporta ADD COLUMN IF
 * NOT EXISTS), statements separados por `--> statement-breakpoint`.
 *
 * Uso:
 *   pnpm tsx scripts/apply-0026-manually.ts
 */
...
const MIGRATION_PATH = path.resolve(
  process.cwd(),
  "drizzle/0026_product_cache_dim_cost.sql",
);
...
const EXPECTED_COLUMNS = [
  ...
  "average_cost",
  "average_landed_cost",
  "weight_updated_local_at",
  "weight_updated_bl_at",
  "dim_updated_local_at",
  "dim_updated_bl_at",
  "cost_updated_local_at",
  "cost_updated_bl_at",
];
...
async function main() {
  ...
  console.log("=== APLICAÇÃO MANUAL DA MIGRATION 0026 ===");
  ...
  console.log(`\n--- Verificação: SHOW COLUMNS FROM ${TARGET_TABLE} ---`);
  ...
  if (allPresent && errCount === 0) {
    console.log("\n✅ Migration 0026 aplicada com sucesso.");
    process.exit(0);
    ...
    console.log("\n❌ Migration 0026 NÃO foi totalmente aplicada — revisar erros.");
    process.exit(1);
  }
}

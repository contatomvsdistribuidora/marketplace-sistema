/**
 * Aplica manualmente a migration 0021 (drizzle/0021_multi_product_listing_items.sql)
 ...
 * migrations 0017/0018/0019/0020 — SQL idempotente (CREATE TABLE IF NOT EXISTS) +
 ...
 *   5. Confirma com SHOW TABLES LIKE 'multi_product_listing_items'.
 *
 * Uso:
 *   pnpm tsx scripts/apply-0021-manually.ts
 *
...
const MIGRATION_PATH = path.resolve(
  process.cwd(),
  "drizzle/0021_multi_product_listing_items.sql",
);

const EXPECTED_TABLES = [
  "multi_product_listing_items",
];
...
async function main() {
  ...
  console.log("=== APLICAÇÃO MANUAL DA MIGRATION 0021 ===");
  ...
  // Confirma criação.
  console.log("\n--- Verificação: SHOW TABLES LIKE 'multi_product_listing_items' ---");
  let presentTables: string[] = [];
  try {
    const [rows] = await conn.query(
      "SHOW TABLES LIKE 'multi_product_listing_items'",
    );
  ...
  if (allPresent && errCount === 0) {
    console.log("\n✅ Migration 0021 aplicada com sucesso.");
    process.exit(0);
    ...
    console.log("\n❌ Migration 0021 NÃO foi totalmente aplicada — revisar erros.");
    process.exit(1);
  }
}

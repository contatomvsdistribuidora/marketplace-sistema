/**
 * Aplica manualmente a migration 0022 (drizzle/0022_video_bank.sql)
 ...
 * migrations 0017/0018/0019/0020/0021 — SQL idempotente (CREATE TABLE IF NOT EXISTS) +
 ...
 *   5. Confirma com SHOW TABLES LIKE 'video_bank'.
 *
 * Uso:
 *   pnpm tsx scripts/apply-0022-manually.ts
 *
...
async function main() {
  ...
  console.log("=== APLICAÇÃO MANUAL DA MIGRATION 0022 ===");
  ...
  // Confirma criação.
  console.log("\n--- Verificação: SHOW TABLES LIKE 'video_bank' ---");
  let presentTables: string[] = [];
  try {
    const [rows] = await conn.query(
      "SHOW TABLES LIKE 'video_bank'",
    );
  ...
  if (allPresent && errCount === 0) {
    ...
    console.log("\n❌ Migration 0022 NÃO foi totalmente aplicada — revisar erros.");
    process.exit(1);
  }
}

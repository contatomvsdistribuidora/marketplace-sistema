/**
 * Aplica manualmente a migration 0019 (drizzle/0019_shopee_shipping_channels.sql)
 ...
 * migrations 0017/0018 — SQL idempotente (CREATE TABLE IF NOT EXISTS) +
 * script tsx ad-hoc, espelhando scripts/apply-0018-manually.ts.
 *
 * O script:
 ...
 *   5. Confirma com SHOW TABLES LIKE 'shopee_shipping_channels'.
 *
 * Uso:
...
const MIGRATION_PATH = path.resolve(
  process.cwd(),
  "drizzle/0019_shopee_shipping_channels.sql",
);

const EXPECTED_TABLES = [
  "shopee_shipping_channels",
];
...
async function main() {
  ...
  console.log("=== APLICAÇÃO MANUAL DA MIGRATION 0019 ===");
  ...
  // Confirma criação.
  console.log("\n--- Verificação: SHOW TABLES LIKE 'shopee_shipping_channels' ---");
  let presentTables: string[] = [];
  try {
    const [rows] = await conn.query(
      "SHOW TABLES LIKE 'shopee_shipping_channels'",
    );
  ...
  if (allPresent && errCount === 0) {
    ...
    console.log("\n❌ Migration 0019 NÃO foi totalmente aplicada — revisar erros.");
    process.exit(1);
  }
}

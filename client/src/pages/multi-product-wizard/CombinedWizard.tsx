              ...
              {optionDetailsMatrix.map((row, productIdx) => {
                ...
                return (
                  <section key={`${product.source}:${product.sourceId}`} className="border rounded-xl overflow-hidden mb-4">
                    ...
                    {/* Lista de cards desta linha (cada card = 1 opcao) */}
                          ...
                          <div key={opt.id} className={`border rounded-xl overflow-hidden ${isNeg ? "border-red-300" : "border-gray-200"}`}>
                            ...
                            <div className={`flex items-center justify-between px-4 py-3 border-b ${isNeg ? "bg-red-50 border-red-200" : "bg-gray-50 border-gray-200"}`}>
                              <div className="flex items-center gap-2 flex-1 min-w-0 mr-2">
                                ...
                                <input
                                  ...
                                  onChange={e => {
                                    ...
                                    updateOptionLabel(opt.id, val, productIdx);
                                    if (idx > 0) {
                              ...
                              <div className="flex items-center gap-2 flex-shrink-0">
                                ...
                                <button onClick={() => suggestDimensions(opt.id, productIdx)}
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <div>
                                  ...
                                  <input
                                    type="text"
                                    maxLength={64}
                                    placeholder={product.sku ? `${product.sku}-${idx + 1}` : "Ex: SAC-15L-100"}
                                    value={opt.sku}
                                ...
                                {row.length === 1 ? (
                                  <div className="sm:col-span-2">
                                    ...
                                    <input
                                      ...
                                      value={opt.ean}
                                      onChange={(e) => updateDetail(opt.id, "ean", e.target.value.replace(/\D/g, ""), productIdx)}
                                  ...
                                  <div className="sm:col-span-2 bg-gray-50 border border-dashed border-gray-300 rounded-lg p-2 text-[11px] text-gray-500">
                                    💡 A Shopee só permite EAN a nível de produto, não por variação. Com {row.length} variações, o EAN fica desabilitado.
                                  </div>
                                )}
                              </div>

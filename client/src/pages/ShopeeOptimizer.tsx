  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-2 py-4 px-6 rounded-xl border-2 border-dashed border-orange-300 text-orange-600 font-semibold text-base hover:bg-orange-50 transition mb-4"
      >
        <Plus className="w-5 h-5" /> Adicionar Variação
      </button>
...
function VariationWizard({
  ...
  // ── Step A helpers ──
  function selectType(type: VariationType) {
  ...
  async function suggestWithAI() {
    try {
      ...
      const suggestions = result.keywords?.slice(0, 5) ?? [];
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-gray-100">
          <h3 className="text-base font-semibold text-gray-900">Adicionar Variação</h3>
        ...
        <div className="flex items-center gap-0 px-6 py-3 border-b border-gray-100">
          {STEPS.map((s, i) => (
            <div key={s.key} className="flex items-center flex-1">
              <div className={`flex items-center gap-1.5 ${step === s.key ? "text-orange-600" : "text-gray-400"}`}>
                <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-all ${
                  step === s.key ? "bg-orange-500 border-orange-500 text-white"
                  : STEPS.indexOf(STEPS.find(x => x.key === step)!) > i ? "bg-orange-100 border-orange-300 text-orange-600"
                  : "bg-gray-100 border-gray-300 text-gray-500"
                }`}>
                  {STEPS.indexOf(STEPS.find(x => x.key === step)!) > i ? <Check className="w-3 h-3" /> : i + 1}
                </div>
                <span className="text-xs font-medium hidden sm:block">{s.label}</span>
              </div>
              {i < STEPS.length - 1 && <div className={`flex-1 h-0.5 mx-2 ${STEPS.indexOf(STEPS.find(x => x.key === step)!) > i ? "bg-orange-300" : "bg-gray-200"}`} />}
            </div>
          ))}
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {VARIATION_TYPES.map(vt => (
                  ...
                  >
                    ...
                    {selectedType === vt.type && <CheckCircle2 className="w-4 h-4 text-orange-500 ml-auto flex-shrink-0 mt-0.5" />}
                  </button>
                ))}
              </div>
            </div>
          ...
          {step === "B" && (
            <div>
              <p className="text-sm text-gray-600 mb-1">Digite cada opção da variação <span className="font-medium text-gray-800">({typeName})</span>:</p>
              <p className="text-xs text-gray-400 mb-4">Ex: "100 unidades", "500 gramas", "Azul"...</p>
              ...
              <div className="border border-dashed border-orange-200 rounded-xl p-4 bg-orange-50">
                <div className="flex items-center justify-between mb-2">
                  <p className="text-sm font-medium text-orange-700">Sugerir opções com IA</p>
                  <button
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {aiSuggestions.map(s => (
                      <button
                        ...
                        className="px-2.5 py-1 bg-white border border-orange-300 text-orange-700 text-xs rounded-full hover:bg-orange-100 transition"
                      >
                        + {s}
                      </button>
                    ))}
                ...
                {optimizeMutation.isError && (
                  <p className="text-xs text-red-500 mt-2">Erro ao buscar sugestões. Tente novamente.</p>
                )}
                {!optimizeMutation.isPending && !optimizeMutation.isError && aiSuggestions.length === 0 && optimizeMutation.isSuccess && (
                  <p className="text-xs text-gray-400 mt-2">Nenhuma sugestão disponível para este produto.</p>
                )}
              </div>
            </div>
          )}
          ...
          {step === "D" && (
            <div>
              <p className="text-sm text-gray-600 mb-4">Revise as variações antes de publicar:</p>
              ...
              <button
                onClick={() => alert("Etapa 4 em breve")}
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex justify-between items-center">
          ...
          {step === "B" && (
            <button
              onClick={goBtoC}
              disabled={optionLabels.every(l => l.trim() === "")}
              className="flex items-center gap-1.5 px-5 py-2.5 bg-orange-500 hover:bg-orange-600 disabled:opacity-40 text-white text-sm font-semibold rounded-xl transition"
            >
              Próximo <ArrowRight className="w-4 h-4" />
            </button>
